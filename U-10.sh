...
CHOWN=$(ls -l /etc/xinetd.conf | awk '{print $3}')

CHECK_PERM=$(find /etc/xinetd.conf -type f -perm -600 -ls | grep -v rw-r--r-- | awk '{print $3}')

CHECK_PERM2=$(ls -l /etc/xinetd.conf | awk '{print $1}')
