...
CHOWN=$(ls -l /etc/syslog.conf | awk '{print $3}')

CHECK_PERM=$(find /etc/syslog.conf -type f -perm -644 -ls | grep -v rw-r--r-- | awk '{print $3}')

CHECK_PERM2=$(ls -l /etc/syslog.conf | awk '{print $1}')
